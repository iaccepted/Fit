//define two ways to calculate the SH coefficients
//1. 勒让德多项式利用theta和phi递归的求解每一项
//2. github上找到的根据x,y,z坐标直接求出N阶SH的各项
#include "SHEval.h"
...
LFLOAT P(int l, int m, LFLOAT x)
{
	...
	if (m>0)
	{
		LFLOAT sqrtOneMinusX2 = sqrt(1.0 - x*x);
...
{
	LFLOAT temp = ((2.0*l + 1.0)*Factorial(l - m)) / ((4.0*M_PI)*Factorial(l + m));
